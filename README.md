...
1. **Runtime** — Claude Code, OpenCode, Gemini, Codex, Copilot, Cursor, Windsurf, Antigravity, Roo Code, or all (interactive multi-select — pick multiple runtimes in a single install session)
...
Use `--claude`, `--opencode`, `--gemini`, `--codex`, `--copilot`, `--cursor`, `--windsurf`, `--antigravity`, `--roo`, or `--all` to skip the runtime prompt.
